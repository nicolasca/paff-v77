...
const Factions: FunctionComponent = () => {
  ...
  React.useEffect(() => {
    axios.get(config.host + ':3008/factions').then((response) => {
      setFactions(response.data.map((faction: IFaction) => {

        let factionComponent = null;

        if (faction.slug === 'peaux-vertes') {
          factionComponent = <FactionPeauxVertes faction={faction}></FactionPeauxVertes>
        }
        if (faction.slug === 'gaeli') {
          factionComponent = <FactionGaeli faction={faction}></FactionGaeli>
        }
        if (faction.slug === 'sephosi') {
          factionComponent = <FactionSephosi faction={faction}></FactionSephosi>
        }
        if (faction.slug === 'liches') {
          factionComponent = <FactionLiches faction={faction}></FactionLiches>
        }
